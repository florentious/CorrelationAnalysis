...
<script type="text/javascript" src="https://www.gstatic.com/charts/loader.js"></script>
    <script type="text/javascript">
      ...
      function drawChart() {
        ...
        %>
        data.addRows([
        	[new Date('<%=dto.getDate().substring(0,4) %>','<%=dto.getDate().substring(5,7) %>', '<%=dto.getDate().substring(8) %>'), <%=dto.getDj_close() %>, <%=dto.getDj_ma5() %>, <%=dto.getDj_ma60() %>]
        ]);
        ...
        var options = {
   		  chart: {
   	          title: "DowJones's close, MA_5, MA_60"
   	      },
   	      width: 900,
   	      height: 500,
	   	   axes: {
  ...
  <div class="container">
      <div class="col-sm-12 row">
      	<div><h3>DowJones's Machine Learning Data</h3></div>

   <%-- input content --%>
			<div id="line_top_x"></div>
			
			
			<h4>DowJones's Machine Learning Predict </h4>
					  	...
					  	<%
							 ...
							 %>
								    ...
								    <tr>			    	
								      ...
								      <td><%=ChangeMark.change(predictDto.getDj_vote()) %></td>								      
								      <td><%=ChangeMark.change(predictDto.getDj_knc()) %></td>								      
								      <td><%=ChangeMark.change(predictDto.getDj_svc()) %></td>								      
								      <td><%=ChangeMark.change(predictDto.getDj_rfc()) %></td>								      
								      <td><%=ChangeMark.change(predictDto.getDj_xgbc()) %></td>								      
								      <td><%=ChangeMark.change(predictDto.getDj_lgbc()) %></td>								      
