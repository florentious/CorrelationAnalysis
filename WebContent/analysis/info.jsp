...
<script type="text/javascript" src="https://www.gstatic.com/charts/loader.js"></script>
    <script type="text/javascript">
      ...
      function drawChart() {
        ...
        %>
        data.addRows([
        	[new Date('<%=dto.getDate().substring(0,4) %>','<%=dto.getDate().substring(5,7) %>', '<%=dto.getDate().substring(8) %>'), <%=dto.getGol_close() %>, <%=dto.getGol_ma5() %>, <%=dto.getGol_ma60() %>]
        ]);
  ...
  <div class="container">
      <div class="col-sm-12 row">
      	<div><h3>Gold's Machine Learning Data</h3></div>

   <%-- input content --%>
			<div id="line_top_x"></div>
			
			
			<h4>Gold's Machine Learning Predict </h4>
					  	...
					  	<%
							 ...
							 %>
								    ...
								    <tr>			    	
								      ...
								      <td><%=ChangeMark.change(predictDto.getGol_vote()) %></td>								      
								      <td><%=ChangeMark.change(predictDto.getGol_knc()) %></td>								      
								      <td><%=ChangeMark.change(predictDto.getGol_svc()) %></td>								      
								      <td><%=ChangeMark.change(predictDto.getGol_rfc()) %></td>								      
								      <td><%=ChangeMark.change(predictDto.getGol_xgbc()) %></td>								      
								      <td><%=ChangeMark.change(predictDto.getGol_lgbc()) %></td>								      
